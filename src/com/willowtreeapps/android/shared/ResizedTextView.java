...
public class ResizedTextView extends TextView{
	...
	public ResizedTextView(Context context, AttributeSet attrs, int defStyle) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs, new int[]{R.attr.maxCustomLines, R.attr.minTextSize});
		maxLines = a.getInt(0, 1);
		minTextSize = a.getInt(1, 8);
	}
...
//		if(tooLarge) {this.setEllipsize(getEllipsize());}
	...
	private int findTextSize(int lines) {
		...
		if(textSize - 2 < minTextSize) {
			if(lines + 1 > maxLines) {return (int) (minTextSize); } else {return findTextSize(lines + 1);}
		} else {
