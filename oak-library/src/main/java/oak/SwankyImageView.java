 ...
 *
 * Usage:
 * - Set image source via android:src, setImageBitmap(), or setImageResource()
 * - Supports scale type of CENTER_CROP (default) and CENTER_INSIDE
 * - Default maximum zoom scale is 2.5x
 *
 * @author Nate Vogt
 * Based loosely on TouchImageView by Michael Ortiz
 */
...
//			log("x = "+x+", y = "+y+", bottom = "+bottom+", right = "+right);
	...
	private class TapSwipeListener extends GestureDetector.SimpleOnGestureListener {
		...
		public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
			mode = NONE;
			mFlingTask = new FlingTask(timeOfLastMoveEvent, velocityX/1000, velocityY/1000).execute();
			return true;
		}
	}
	...
	private class ScaleListener extends
		...
		public boolean onScale(ScaleGestureDetector detector) {
			...
			if (viewWiderThanImg || viewTallerThanImg) {
				if(viewWiderThanImg && !viewTallerThanImg)
					matrix.postScale(mScaleFactor, mScaleFactor, viewWidth / 2, detector.getFocusY());
				else if(!viewWiderThanImg && viewTallerThanImg)
					matrix.postScale(mScaleFactor, mScaleFactor, detector.getFocusX(), viewHeight / 2);
				else
